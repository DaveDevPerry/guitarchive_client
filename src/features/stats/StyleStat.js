...
const StyleStat = ({ stat }) => {
	const { width } = useViewport();
	const breakpoint = 620;
	return (
		<>
			{width < breakpoint ? (
				<StyledStyleStatMobile>
						...
						<div className='stat-icon-wrapper'>
							{stat.statName === 'fingerstyle' && (
								<Tooltip content='fingerstyle' direction='left'>
									<IoHandLeftSharp className='status-icon' />
								</Tooltip>
							)}
							{stat.statName === 'classical' && (
								<Tooltip content='classical' direction='left'>
									<SiStylelint className='status-icon' />
								</Tooltip>
							)}
						</div>
					</div>
...
const StyledStyleStatMobile = styled.div`
...
`;
const StyledStyleStat = styled.div`
	/* border: 1px solid green; */
	border-radius: 1rem;
	padding: 1rem 2rem;
	display: flex;
