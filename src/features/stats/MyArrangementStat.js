...
const MyArrangementStat = ({ stat }) => {
	const { width } = useViewport();
	const breakpoint = 620;
	return (
		<>
			{width < breakpoint ? (
				<StyledMyArrangementStatMobile>
					<div className='stat-header'>
						...
						<div className='stat-icon-wrapper'>
							{stat.statName === 'terrorvision' && (
								<Tooltip content='TERRORVISION' direction='left'>
									<ImUsers className='status-icon' />
								</Tooltip>
							)}
							{stat.statName === 'other' && (
								<Tooltip content='all other arrangements' direction='left'>
									<ImUsers className='status-icon' />
								</Tooltip>
							)}
						</div>
...
const StyledMyArrangementStatMobile = styled.div`
...
`;
const StyledMyArrangementStat = styled.div`
	/* border: 1px solid green; */
	border-radius: 1rem;
	padding: 1rem 2rem;
	display: flex;
...
export default MyArrangementStat;
